...
image_label = '**IMAGE**'
video_label = '**VIDEO**'
input_file = 'thread2.tsv'
output_name = 'stat_post_2'
output_extension = '.tsv'
output_file = output_name + output_extension

keywords = ['гитар', 'струн', 'строй', 'гриф', 'анкер', 'лад', 'мензур', 'тремол']
...
def get_reply_count(data, authors_pattern,
    ...
    for row in data:
        ...
        if 'Цитата: ' in text:
            reply_count = text.count('Цитата: ')

            if reply_count > 1:
                pass

            for i in range(0, reply_count):
                raw_reply = text.partition('Цитата: ')[2]
                reply = raw_reply.partition('Цитата: ')[0].strip()
                matches = re.match('(' + authors_pattern + ')( от [а-яА-Я]+ \d{1,2}, \d+, \d+:\d+:\d+)', reply)

                if matches:
                    author = matches.group(1)
                    date = matches.group(2)
                    message = reply.partition(author + date)[2]
                    words = message.split()
                    if len(words) < 3:
                        continue
                    else:
...
def main():
    ...
    reply_count = original_quoting(authors, authors_index, authors_pattern, data)
    reply_count_naive_quoted = naive_quoting(authors, authors_index, data)

    positions = get_positions(data)
    sentiment = get_sentiment(data)
    keywords_count = get_keywords_count(data)
    ...
    print_to_csv(post_statistics, output_name + output_extension)
    print_to_csv(post_statistics_naive_quoted, output_name + '_naive_quoted' + output_extension)
    print_to_csv(num_of_threads, output_name + '_threads_count' + output_extension)


def get_post_stat(keywords_count, links_count, positions, reply_count, sentiment, utility, words_count):
    post_statistics = list(zip(words_count, links_count,
                               reply_count, positions,
                               sentiment, keywords_count, utility))
...
def naive_quoting(authors, authors_index, data):
    # noinspection PyUnusedLocal
    ...
    print_to_csv(make_matrix_graph(naive_quoted_social_graph_clean_raw, authors),
                 output_name + '_naive_quoted_soc_graph_clean_raw' + output_extension)
    print_to_csv(make_matrix_graph(naive_quoted_social_graph_clean_normalized, authors),
                 output_name + '_naive_quoted_soc_graph_clean_normalized' + output_extension)
    print_to_csv(make_matrix_graph(naive_quoted_social_graph_sentim_raw, authors),
                 output_name + '_naive_quoted_soc_graph_sentim_raw' + output_extension)
    print_to_csv(make_matrix_graph(naive_quoted_social_graph_sentim_normalized, authors),
                 output_name + '_naive_quoted_soc_graph_sentim_normalized' + output_extension)

    return reply_count_naive_quoted


def original_quoting(authors, authors_index, authors_pattern, data):
    # noinspection PyUnusedLocal
    ...
    print_to_csv(make_matrix_graph(social_graph_clean_raw, authors),
                 output_name + '_soc_graph_clean_raw' + output_extension)
    print_to_csv(make_matrix_graph(social_graph_clean_normalized, authors),
                 output_name + '_soc_graph_clean_normalized' + output_extension)
    print_to_csv(make_matrix_graph(social_graph_sentim_raw, authors),
                 output_name + '_soc_graph_sentim_raw' + output_extension)
    print_to_csv(make_matrix_graph(social_graph_sentim_normalized, authors),
                 output_name + '_soc_graph_sentim_normalized' + output_extension)
